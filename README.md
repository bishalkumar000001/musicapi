...
## JunoXMusic PRO / Heroku

This fork is prepared as the **JunoXMusic KuGou PRO API**. It keeps the original KuGou endpoints and adds Heroku-ready startup, a health endpoint, a root status endpoint, and optional API-key authentication.

### Heroku

Use the **Node.js buildpack** (or deploy with the included Dockerfile). Heroku automatically supplies `PORT`; the server listens on `0.0.0.0`.
...
```text
API_NAME=JunoXMusic KuGou PRO API
CORS_ALLOW_ORIGIN=*
```

Authentication can be sent as either:
...
All other API endpoints require `API_KEY` when it is configured.
