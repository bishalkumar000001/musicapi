...
async function consturctServer(moduleDefs) {
  ...
  // ============================================================
  // JunoXMusic PRO API security + health endpoints
  // ============================================================
  // Set API_KEY to protect API routes. Health/root remain public.
  const configuredApiKey = (process.env.API_KEY || '').trim();
  const apiKeyHeaderNames = ['x-api-key', 'x-api-key-token'];

  app.get('/health', (_req, res) => {
    res.json({
      status: 'ok',
      service: process.env.API_NAME || 'JunoXMusic KuGou PRO API',
      version: process.env.API_VERSION || '2.0.0',
      uptime: Math.floor(process.uptime()),
      timestamp: new Date().toISOString(),
      authentication: configuredApiKey ? 'api_key_required' : 'disabled',
    });
  });
  ...
  app.get('/', (_req, res) => {
    res.json({
      status: 'ok',
      name: process.env.API_NAME || 'JunoXMusic KuGou PRO API',
      version: process.env.API_VERSION || '2.0.0',
      message: 'JunoXMusic KuGou API is running',
      docs: '/docs',
      health: '/health',
      authentication: configuredApiKey ? 'API key required for API routes' : 'disabled',
    });
  });

  if (configuredApiKey) {
    app.use((req, res, next) => {
      // Public infrastructure/docs endpoints.
      if (req.path === '/' || req.path === '/health' || req.path.startsWith('/docs')) return next();

      let provided = '';
      for (const name of apiKeyHeaderNames) {
        if (req.headers[name]) {
          provided = String(req.headers[name]).trim();
          break;
        }
      }

      // Also accept Authorization: Bearer <API_KEY>.
      if (!provided) {
        const authorization = String(req.headers.authorization || '');
        if (/^Bearer\s+/i.test(authorization)) {
          provided = authorization.replace(/^Bearer\s+/i, '').trim();
        }
      }
